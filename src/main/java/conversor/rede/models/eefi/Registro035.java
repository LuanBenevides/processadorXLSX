package conversor.rede.models.eefi;

public class Registro035 {

	public String tipoRegistro;
	public String numPvAjustado;
	public String numRvAjustado;
	public String dataAjuste;
	public String valorAjuste;
	public String debito;
	public String motivoAjuste;
	public String motivoAjusteTxt;
	public String numCartao;
	public String dataTransacaoCv;
	public String numRvOriginal;
	public String numRefCartaFax;
	public String dataCarta;
	public String mesRef;
	public String numPvOriginal;
	public String dataRvOriginal;
	public String valorTransacao;
	public String desagendamentoOuNet;
	public String dataCredito;
	public String novoValorParcela;
	public String valorOriginalParcela;
	public String valorBrutoRvOriginal;
	public String valorCancelamentoSolicitado;
	public String numNsu;
	public String numAutorizacao;
	public String tipoDebito;
	public String numOrdemDebito;
	public String valorDebitoTotal;
	public String valorPedente;
	public String bandeiraRvOrigem;
	public String bandeiraRvAjustado;
	public String numParcelaRvAjustado;
	public String numParcelaRvOriginal;
	public String dataRvAjustado;
	
	//Getters and Setters methods
	public String getTipoRegistro() {
		return tipoRegistro;
	}
	public void setTipoRegistro(String tipoRegistro) {
		this.tipoRegistro = tipoRegistro;
	}
	public String getNumPvAjustado() {
		return numPvAjustado;
	}
	public void setNumPvAjustado(String numPvAjustado) {
		this.numPvAjustado = numPvAjustado;
	}
	public String getNumRvAjustado() {
		return numRvAjustado;
	}
	public void setNumRvAjustado(String numRvAjustado) {
		this.numRvAjustado = numRvAjustado;
	}
	public String getDataAjuste() {
		return dataAjuste;
	}
	public void setDataAjuste(String dataAjuste) {
		this.dataAjuste = dataAjuste;
	}
	public String getValorAjuste() {
		return valorAjuste;
	}
	public void setValorAjuste(String valorAjuste) {
		this.valorAjuste = valorAjuste;
	}
	public String getDebito() {
		return debito;
	}
	public void setDebito(String debito) {
		this.debito = debito;
	}
	public String getMotivoAjuste() {
		return motivoAjuste;
	}
	public void setMotivoAjuste(String motivoAjuste) {
		this.motivoAjuste = motivoAjuste;
	}
	public String getMotivoAjusteTxt() {
		return motivoAjusteTxt;
	}
	public void setMotivoAjusteTxt(String motivoAjusteTxt) {
		this.motivoAjusteTxt = motivoAjusteTxt;
	}
	public String getNumCartao() {
		return numCartao;
	}
	public void setNumCartao(String numCartao) {
		this.numCartao = numCartao;
	}
	public String getDataTransacaoCv() {
		return dataTransacaoCv;
	}
	public void setDataTransacaoCv(String dataTransacaoCv) {
		this.dataTransacaoCv = dataTransacaoCv;
	}
	public String getNumRvOriginal() {
		return numRvOriginal;
	}
	public void setNumRvOriginal(String numRvOriginal) {
		this.numRvOriginal = numRvOriginal;
	}
	public String getNumRefCartaFax() {
		return numRefCartaFax;
	}
	public void setNumRefCartaFax(String numRefCartaFax) {
		this.numRefCartaFax = numRefCartaFax;
	}
	public String getDataCarta() {
		return dataCarta;
	}
	public void setDataCarta(String dataCarta) {
		this.dataCarta = dataCarta;
	}
	public String getMesRef() {
		return mesRef;
	}
	public void setMesRef(String mesRef) {
		this.mesRef = mesRef;
	}
	public String getNumPvOriginal() {
		return numPvOriginal;
	}
	public void setNumPvOriginal(String numPvOriginal) {
		this.numPvOriginal = numPvOriginal;
	}
	public String getDataRvOriginal() {
		return dataRvOriginal;
	}
	public void setDataRvOriginal(String dataRvOriginal) {
		this.dataRvOriginal = dataRvOriginal;
	}
	public String getValorTransacao() {
		return valorTransacao;
	}
	public void setValorTransacao(String valorTransacao) {
		this.valorTransacao = valorTransacao;
	}
	public String getDesagendamentoOuNet() {
		return desagendamentoOuNet;
	}
	public void setDesagendamentoOuNet(String desagendamentoOuNet) {
		this.desagendamentoOuNet = desagendamentoOuNet;
	}
	public String getDataCredito() {
		return dataCredito;
	}
	public void setDataCredito(String dataCredito) {
		this.dataCredito = dataCredito;
	}
	public String getNovoValorParcela() {
		return novoValorParcela;
	}
	public void setNovoValorParcela(String novoValorParcela) {
		this.novoValorParcela = novoValorParcela;
	}
	public String getValorOriginalParcela() {
		return valorOriginalParcela;
	}
	public void setValorOriginalParcela(String valorOriginalParcela) {
		this.valorOriginalParcela = valorOriginalParcela;
	}
	public String getValorBrutoRvOriginal() {
		return valorBrutoRvOriginal;
	}
	public void setValorBrutoRvOriginal(String valorBrutoRvOriginal) {
		this.valorBrutoRvOriginal = valorBrutoRvOriginal;
	}
	public String getValorCancelamentoSolicitado() {
		return valorCancelamentoSolicitado;
	}
	public void setValorCancelamentoSolicitado(String valorCancelamentoSolicitado) {
		this.valorCancelamentoSolicitado = valorCancelamentoSolicitado;
	}
	public String getNumNsu() {
		return numNsu;
	}
	public void setNumNsu(String numNsu) {
		this.numNsu = numNsu;
	}
	public String getNumAutorizacao() {
		return numAutorizacao;
	}
	public void setNumAutorizacao(String numAutorizacao) {
		this.numAutorizacao = numAutorizacao;
	}
	public String getTipoDebito() {
		return tipoDebito;
	}
	public void setTipoDebito(String tipoDebito) {
		this.tipoDebito = tipoDebito;
	}
	public String getNumOrdemDebito() {
		return numOrdemDebito;
	}
	public void setNumOrdemDebito(String numOrdemDebito) {
		this.numOrdemDebito = numOrdemDebito;
	}
	public String getValorDebitoTotal() {
		return valorDebitoTotal;
	}
	public void setValorDebitoTotal(String valorDebitoTotal) {
		this.valorDebitoTotal = valorDebitoTotal;
	}
	public String getValorPedente() {
		return valorPedente;
	}
	public void setValorPedente(String valorPedente) {
		this.valorPedente = valorPedente;
	}
	public String getBandeiraRvOrigem() {
		return bandeiraRvOrigem;
	}
	public void setBandeiraRvOrigem(String bandeiraRvOrigem) {
		this.bandeiraRvOrigem = bandeiraRvOrigem;
	}
	public String getBandeiraRvAjustado() {
		return bandeiraRvAjustado;
	}
	public void setBandeiraRvAjustado(String bandeiraRvAjustado) {
		this.bandeiraRvAjustado = bandeiraRvAjustado;
	}
	public String getNumParcelaRvAjustado() {
		return numParcelaRvAjustado;
	}
	public void setNumParcelaRvAjustado(String numParcelaRvAjustado) {
		this.numParcelaRvAjustado = numParcelaRvAjustado;
	}
	public String getNumParcelaRvOriginal() {
		return numParcelaRvOriginal;
	}
	public void setNumParcelaRvOriginal(String numParcelaRvOriginal) {
		this.numParcelaRvOriginal = numParcelaRvOriginal;
	}
	public String getDataRvAjustado() {
		return dataRvAjustado;
	}
	public void setDataRvAjustado(String dataRvAjustado) {
		this.dataRvAjustado = dataRvAjustado;
	}
}
